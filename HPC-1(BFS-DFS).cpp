#include <omp.h> // to execute code in multiple threads
#include <iostream>
#include <queue>
#include <stack>
#include <chrono> // for bench marking
using namespace std;
...
void dfs(Node *root)
{
    if (root == NULL)
        return;
#pragma omp critical     // ensures below line prints by only 1 thread at a time only once.
    cout << root->value << "  ";
#pragma omp parallel sections num_threads(2) // multithreading - to execute the dfs recursively for left and right child using 2 threads parallely.
    {
#pragma omp section
        dfs(root->left);
#pragma omp section
